...
conftest.py sets WEBHOOK_TOKEN to "test-webhook-token" before anything imports
app.config, so that is the secret under test throughout this file.
...
@pytest.mark.parametrize(
    ("supplied", "case"),
    [
        ("test-webhook-tokeN", "final character differs"),
        ("test-webhook-toke", "a correct prefix, one character short"),
        ("test-webhook-token-extra", "correct token plus a suffix"),
        ("", "empty string"),
        (None, "no token key in the body at all"),
        (12345, "a JSON number rather than a string"),
        ([VALID], "the correct token, wrapped in a list"),
        ("tëst-webhook-token", "non-ASCII"),
    ],
)
